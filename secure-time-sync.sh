...
#
#    Changes:
#    12/2023 - marin-ante - added timeout, fixed parsing, removed duckduckgo
#
...
select_pool() {
  ...
  # does not work reliably with vpn
  # DuckDuckGo.
  #if [ "${use_tor}" = "true" ]; then
  ...
  # EFF.
  POOL[4]="https://www.eff.org"

  # The last one doesn't get selected. Without the following line, POOL[4] would never be selected.
  POOL[5]=""
...
if [ "${use_tor}" = "true" ]; then
  ...
  SECURE_CURL="curl -sI --socks5-hostname localhost:9050 --connect-timeout 3"
else
  # Protects against https downgrade attacks when not using Tor.
  SECURE_CURL="curl -sI --tlsv1.2 --proto =https --connect-timeout 3"
fi
